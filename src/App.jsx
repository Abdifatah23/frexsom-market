...
const PRODUCTS = [
  ...
  { id:9, name:"Prayer Mat Premium", category:"Fashion", price:22.00, oldPrice:30.00, cost:6.00, stock:180, rating:4.9, reviews:210, badge:"Best Seller", isNew:false,
    description:"Soft, thick prayer mat with beautiful geometric pattern — ideal for daily prayers.",
    image:"https://images.unsplash.com/photo-1584551246679-0daf3d275d0f?w=600&q=80" },
  { id:10, name:"Perfume Gift Set", category:"Beauty", price:35.00, oldPrice:48.00, cost:10.00, stock:95, rating:4.7, reviews:88, badge:"Popular", isNew:false,
    description:"Luxurious fragrance gift set with three long-lasting scents — perfect for any occasion.",
    image:"https://images.unsplash.com/photo-1541643600914-78b084683702?w=600&q=80" },
  { id:11, name:"Skin Care Set", category:"Beauty", price:42.00, oldPrice:58.00, cost:12.00, stock:70, rating:4.5, reviews:64, badge:"New", isNew:true,
    description:"Complete daily skincare routine set — cleanser, toner and moisturizer for all skin types.",
    image:"https://images.unsplash.com/photo-1556228578-8c89e6adf883?w=600&q=80" },
  { id:12, name:"Football & Sports Kit", category:"Sports", price:28.00, oldPrice:38.00, cost:8.00, stock:130, rating:4.4, reviews:45, badge:"", isNew:false,
    description:"Complete sports kit with football, pump and training cones — suitable for all ages.",
...
export default function App() {
  ...
  const doSource = async () => {
    ...
    const txt = await ask(
      [{ role:"user", content:`I run an e-commerce store in Somalia. Find me 4 products I can source and sell for: "${srcQ}". Focus on quality, demand and profitability.` }],
      `You are a global product sourcing expert for an e-commerce business. Return ONLY a valid JSON array of 4 products. Each object must have: name (string), supplier (string - type of supplier e.g. "Wholesale Manufacturer"), moq (number - minimum order), unitCost (number in USD), estimatedShipping (number in USD per unit), category (string), whyItSells (string - 1 sentence on why this product has high demand), profitMargin (string - estimated margin %). No markdown, no explanation, just the JSON array.`
    );
    try { setSrcRes(JSON.parse(txt.replace(/```json|```/g,"").trim())); }
    catch { setSrcRes([]); }
    setSrcLoad(false);
  };
  ...
  const importP = (p) => {
    const newProduct = {
      id:Date.now(), name:p.name, category:p.category,
      price:parseFloat(((p.unitCost+p.estimatedShipping)*2.5).toFixed(2)),
      oldPrice:null, cost:parseFloat((p.unitCost+p.estimatedShipping).toFixed(2)),
      stock:p.moq, rating:4.5, reviews:0, badge:"New", isNew:true,
      description:p.whyItSells,
  ...
  const rewriteDesc = async (product) => {
    ...
    const txt = await ask(
      [{ role:"user", content:`Write a compelling 2-sentence product description for: ${product.name} (${product.category}), priced at $${product.price}.` }],
      `You write short, persuasive e-commerce product descriptions. Focus on benefits, quality and value. Return only the description text, no quotes or formatting.`
    );
  ...
  const doPrice = async () => {
    ...
    const txt = await ask(
      [{ role:"user", content:`Product: ${pName}. My total landed cost: $${pCost}. Suggest optimal pricing for a Somali e-commerce market.` }],
  ...
  const doChat = async () => {
    ...
    const txt = await ask(nm, `You are a helpful customer support agent for FrexSOM Market — a leading online store in Somalia. Help customers with: shipping (15–30 days), payments (EVC Plus, Zaad, E-Dahab, cash on delivery), returns (30 days), and product questions. Contact: info@frexsom.com | (+252) 611 013 604. Be warm, professional, and concise — 2–3 sentences max per reply.`);
    setMsgs(prev=>[...prev,{role:"assistant",content:txt||"Sorry, I ran into an issue. Please call (+252) 611 013 604 for immediate help."}]);
    setChatLoad(false);
  };

  const filteredProducts = products.filter(p=>(catFilter==="all"||p.category===catFilter)&&(searchQ===""||p.name.toLowerCase().includes(searchQ.toLowerCase())||p.category.toLowerCase().includes(searchQ.toLowerCase())||p.description.toLowerCase().includes(searchQ.toLowerCase())));
            ...
            <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr" }}>
              ...
              <div style={{ padding:26,display:"flex",flexDirection:"column" }}>
                ...
                <div style={{ display:"flex",gap:8 }}>
                  ...
                  <button onClick={()=>toggleWish(quickView.id)} style={{ background:wishlist.includes(quickView.id)?"#fef2f2":"#f0f2f8",border:"none",padding:"11px 14px",borderRadius:9,cursor:"pointer",fontSize:17 }}>{wishlist.includes(quickView.id)?"❤️":"🤍"}</button>
      ...
      <div style={{ flex:1,overflowY:"auto",background:GRAY }}>
          ...
          <div style={{ background:"#f8fafc",minHeight:"100%" }}>
            ...
            <div style={{ background:NAVY,padding:"0 24px",position:"sticky",top:0,zIndex:20,boxShadow:"0 2px 16px rgba(0,0,0,0.2)" }}>
              <div style={{ display:"flex",alignItems:"center",gap:14,height:66 }}>
                ...
                <div style={{ flex:1,maxWidth:520,position:"relative" }}>
                  ...
                  {searchInput&&(
                    <button onClick={clearSearch} style={{ position:"absolute",right:14,top:"50%",transform:"translateY(-50%)",background:"none",border:"none",cursor:"pointer",color:"#94a3b8",fontSize:16,display:"flex",alignItems:"center" }}>✕</button>
                  )}
                </div>
              ...
              <div style={{ display:"flex",gap:6,paddingBottom:10,overflowX:"auto" }}>
                {CATEGORIES.map(c=>(
                  <button key={c.id} onClick={()=>{ setCatFilter(c.id); setSearchQ(""); setSearchInput(""); setTimeout(()=>productsRef.current?.scrollIntoView({behavior:'smooth'}),100); }} style={{ background:catFilter===c.id?ORANGE:"rgba(255,255,255,0.1)",border:`1px solid ${catFilter===c.id?ORANGE:"rgba(255,255,255,0.15)"}`,color:"white",padding:"5px 18px",borderRadius:20,cursor:"pointer",fontWeight:catFilter===c.id?700:400,fontSize:12.5,whiteSpace:"nowrap",fontFamily:"inherit" }}>
                    {c.label}
                  </button>
                ))}
              </div>
            ...
            {searchQ&&(
              <div style={{ background:`${NAVY}08`,borderBottom:`2px solid ${NAVY}15`,padding:"12px 28px",display:"flex",alignItems:"center",justifyContent:"space-between" }}>
                <div style={{ fontSize:14,color:NAVY,fontWeight:600 }}>
                  🔍 Search results for "<strong>{searchQ}</strong>" — {filteredProducts.length} product{filteredProducts.length!==1?"s":""} found
                </div>
                <button onClick={clearSearch} style={{ background:NAVY,color:"white",border:"none",padding:"6px 16px",borderRadius:20,cursor:"pointer",fontSize:12.5,fontWeight:600,fontFamily:"inherit" }}>✕ Clear Search</button>
              </div>
            )}
            ...
            {!searchQ&&(
              <div style={{ background:`linear-gradient(130deg,${NAVY} 0%,#1a3580 55%,${ORANGE}aa 100%)`,padding:"52px 40px",color:"white",display:"flex",justifyContent:"space-between",alignItems:"center" }}>
                <div style={{ maxWidth:560 }}>
                  ...
                  <div style={{ fontSize:38,fontWeight:900,lineHeight:1.15,marginBottom:14 }}>
                    Shop Smarter.<br/><span style={{ color:ORANGE }}>Save More.</span>
                  </div>
              ...
              <div style={{ padding:"28px 28px 0" }}>
                <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:18 }}>
                  <div style={{ display:"flex",alignItems:"center",gap:14 }}>
                    ...
                    <div style={{ background:NAVY,color:"white",padding:"5px 14px",borderRadius:25,fontSize:13,fontWeight:700 }}>
                      Ends: {pad(countdown.h)}:{pad(countdown.m)}:{pad(countdown.s)}
                    </div>
                  </div>
            ...
            <div ref={productsRef} style={{ padding:"0 28px 32px" }}>
              ...
              {filteredProducts.length===0
                ? <div style={{ textAlign:"center",padding:60,color:"#94a3b8" }}>
                    ...
                    <div style={{ fontSize:13,marginBottom:20 }}>Try a different search term or browse by category</div>
                    <button onClick={clearSearch} style={{ background:NAVY,color:"white",border:"none",padding:"10px 24px",borderRadius:25,cursor:"pointer",fontWeight:700,fontFamily:"inherit" }}>Browse All Products</button>
                  </div>
            ...
            <div style={{ margin:"0 28px 32px",background:NAVY,borderRadius:16,padding:"36px 40px",display:"flex",justifyContent:"space-between",alignItems:"center",gap:20,flexWrap:"wrap" }}>
              ...
              {newsletterDone
                ...
                : <div style={{ display:"flex",gap:10 }}>
                    <input value={newsletter} onChange={e=>setNewsletter(e.target.value)} onKeyDown={e=>e.key==="Enter"&&newsletter.includes("@")&&(setNewsletterDone(true),showToast("Subscribed! 🎉"))} placeholder="Your email address" style={{ padding:"11px 18px",borderRadius:25,border:"none",fontSize:14,outline:"none",width:260,fontFamily:"inherit" }}/>
            ...
            <div style={{ background:NAVY,color:"rgba(255,255,255,0.55)",padding:"44px 36px 24px" }}>
              <div style={{ display:"grid",gridTemplateColumns:"1.6fr 1fr 1fr 1fr",gap:30,marginBottom:32 }}>
                ...
                {[
                  { title:"Quick Links", items:[["Shop All",()=>{ setCatFilter("all"); }],["Track Order",()=>setPage("orders")],["Customer Support",()=>setPage("support")],["Dashboard",()=>setPage("dashboard")]] },
        ...
        {page==="dashboard"&&(
          <div style={{ padding:30 }}>
            <div style={{ marginBottom:24 }}><div style={{ fontSize:24,fontWeight:800,color:NAVY }}>Welcome back 👋</div><div style={{ color:"#7a86a0",fontSize:14,marginTop:3 }}>Your FrexSOM store overview.</div></div>
        ...
        {page==="sourcing"&&(
          <div style={{ padding:30 }}>
            <div style={{ marginBottom:22 }}>
              <div style={{ fontSize:22,fontWeight:800,color:NAVY }}>🔎 Source Products</div>
              <div style={{ color:"#7a86a0",fontSize:14,marginTop:3 }}>Describe what you want to sell — our smart system finds the best products and supplier details for you.</div>
            </div>

            {/* How it works */}
            <div style={{ display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:14,marginBottom:24 }}>
              {[["1️⃣","Describe","Type what product category you want to sell"],["2️⃣","Analyze","Our system finds the best products with pricing"],["3️⃣","Review","Check profit margins and supplier details"],["4️⃣","Import","Add directly to your catalog with one click"]].map(([num,title,desc])=>(
            ...
            <Card style={{ marginBottom:20,padding:24 }}>
              ...
              <div style={{ display:"flex",gap:10,marginBottom:14 }}>
                <input value={srcQ} onChange={e=>setSrcQ(e.target.value)} onKeyDown={e=>e.key==="Enter"&&doSource()}
                  placeholder='e.g. "women fashion accessories", "home appliances", "children school supplies"'
            ...
            {srcLoad&&(
              <Card style={{ padding:50,textAlign:"center" }}>
                ...
                <div style={{ fontWeight:700,color:NAVY,fontSize:16,marginBottom:6 }}>Finding the best products for you…</div>
                <div style={{ fontSize:13,color:"#7a86a0" }}>Analyzing supplier options, profit margins and market demand</div>
              </Card>
            )}

            {srcRes&&srcRes.length===0&&(
              <Card style={{ padding:30,textAlign:"center" }}>
                <div style={{ color:"#ef4444",fontSize:15,fontWeight:600 }}>No results found. Please try a different search term.</div>
              </Card>
            )}
                  ...
                  {srcRes.map((p,i)=>(
                    <Card key={i} style={{ padding:22 }}>
                      <div style={{ display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:16 }}>
                        <div>
                          <div style={{ fontSize:16,fontWeight:800,color:NAVY,marginBottom:3 }}>{p.name}</div>
                          <div style={{ fontSize:12.5,color:"#7a86a0" }}>Supplier type: {p.supplier}</div>
                        </div>
                      ...
                      <div style={{ background:GRAY,borderRadius:10,padding:"14px 16px",marginBottom:14 }}>
                        ...
                        <div style={{ marginTop:10,display:"flex",justifyContent:"space-between",fontSize:12.5 }}>
                          <span style={{ color:"#7a86a0" }}>Min. Order Qty:</span>
                          <span style={{ fontWeight:700,color:NAVY }}>{p.moq} units</span>
                        </div>
                        <div style={{ display:"flex",justifyContent:"space-between",fontSize:12.5,marginTop:4 }}>
                          <span style={{ color:"#7a86a0" }}>Suggested Sell Price:</span>
                          <span style={{ fontWeight:800,color:ORANGE }}>${((p.unitCost+p.estimatedShipping)*2.5).toFixed(2)}</span>
                        </div>
                      </div>

                      <div style={{ background:LIGHT,border:`1px solid ${ORANGE}33`,borderRadius:8,padding:"10px 14px",marginBottom:16,fontSize:13,color:"#7a4000" }}>
                        📈 <strong>Why it sells:</strong> {p.whyItSells}
                      </div>

                      <button onClick={()=>importP(p)} style={{ width:"100%",background:NAVY,color:"white",border:"none",padding:"12px",borderRadius:9,cursor:"pointer",fontWeight:700,fontSize:14,fontFamily:"inherit" }}>
          ...
          <div style={{ padding:30 }}>
            ...
            <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:20,alignItems:"start" }}>
              <Card style={{ padding:24 }}>
                {[["Product Name","text",pName,setPName,"e.g. Solar Power Bank 20000mAh"],["Total Cost in USD (product + shipping)","number",pCost,setPCost,"e.g. 12.50"]].map(([lbl,type,val,set,ph])=>(
                ...
                <div style={{ marginTop:16,padding:"12px 14px",background:`${ORANGE}0e`,borderRadius:9,fontSize:12.5,color:"#7a4000",lineHeight:1.5 }}>💡 Total cost = product price + shipping to Somalia.</div>
              </Card>
              {priceRes&&(
                <div>
            ...
            <Card style={{ flex:1,display:"flex",flexDirection:"column",overflow:"hidden",padding:0 }}>
              ...
              <div style={{ padding:"13px 20px",borderTop:"1px solid #f0f2f8" }}>
                <div style={{ display:"flex",gap:7,marginBottom:10,flexWrap:"wrap" }}>
                  {["Shipping time?","Payment options?","Return policy?","Delivery to Baidoa?","Track my order?"].map(q=>(
