...
const GRAY = "#f4f6fb";
const GREEN = "#10b981";
const ADMIN_PIN = "1604"; // Change this to your secret PIN
...
const PRODUCTS = [
  ...
  { id:7, name:"Portable Blender", category:"Kitchen", price:24.00, oldPrice:32.00, cost:7.00, stock:110, rating:4.4, reviews:38, badge:"New", isNew:true, description:"Compact USB-rechargeable blender — perfect for smoothies and shakes on the go.", image:"https://images.unsplash.com/photo-1570222094114-d054a817e56b?w=600&q=80" },
  ...
  { id:11, name:"Skin Care Set", category:"Beauty", price:42.00, oldPrice:58.00, cost:12.00, stock:70, rating:4.5, reviews:64, badge:"New", isNew:true, description:"Complete daily skincare routine — cleanser, toner and moisturizer for all skin types.", image:"https://images.unsplash.com/photo-1556228578-8c89e6adf883?w=600&q=80" },
  { id:12, name:"Football & Sports Kit", category:"Sports", price:28.00, oldPrice:38.00, cost:8.00, stock:130, rating:4.4, reviews:45, badge:"", isNew:false, description:"Complete sports kit with football, pump and training cones — for all ages.", image:"https://images.unsplash.com/photo-1575361204480-aadea25e6e68?w=600&q=80" },
];
...
const Logo = ({size=20,white=false}) => (
  <div style={{display:"flex",alignItems:"center",gap:7,background:white?"transparent":"white",padding:white?"0":"5px 14px",borderRadius:white?0:30,boxShadow:white?"none":"0 2px 8px rgba(0,0,0,0.12)"}}>
    ...
    <div style={{display:"none",alignItems:"center",gap:4}}>
      ...
      <span style={{fontSize:size,fontWeight:900}}><span style={{color:white?"white":NAVY}}>Frex</span><span style={{color:ORANGE}}>SOM</span></span>
    </div>
  </div>
);
...
export default function App() {
  ...
  const [msgs, setMsgs] = useState([{role:"assistant",content:"Salaam! 👋 Welcome to FrexSOM Market. I'm here to help with orders, delivery, payments and any questions. How can I help?"}]);
  ...
  useEffect(()=>{
    const t=setInterval(()=>{
      setCountdown(prev=>{
        let{h,m,s}=prev; s--; if(s<0){s=59;m--;} if(m<0){m=59;h--;} if(h<0){h=23;}
        return{h,m,s};
      });
    },1000);
  ...
  const handleLogoClick = () => {
    ...
    if(newCount>=5){ setShowAdminLogin(true); setLogoClicks(0); return; }
    logoClickTimer.current = setTimeout(()=>setLogoClicks(0), 3000);
  };

  const tryAdminLogin = () => {
    if(adminPin===ADMIN_PIN){ setIsAdmin(true); setShowAdminLogin(false); setAdminPin(""); showToast("Welcome back, Admin! 👋"); }
  ...
  const handleSearch = () => { if(!searchInput.trim())return; setSearchQ(searchInput); setCatFilter("all"); setTimeout(()=>productsRef.current?.scrollIntoView({behavior:'smooth'}),100); };
  ...
  const doSource = async()=>{
    if(!srcQ.trim())return; setSrcLoad(true); setSrcRes(null);
    const txt=await ask([{role:"user",content:`Find 4 products to source and sell for: "${srcQ}". Focus on quality, demand and profitability.`}],
  ...
  const importP = (p) => {
    ...
    showToast(`${p.name} added to catalog! 📦`); setAdminPage("catalog");
  };

  const rewriteDesc = async(product)=>{
    setDescLoad(product.id);
    const txt=await ask([{role:"user",content:`Write a compelling 2-sentence product description for: ${product.name} ($${product.price}).`}],`Write short, persuasive e-commerce product descriptions. Return only the description text.`);
    if(txt) setProducts(prev=>prev.map(p=>p.id===product.id?{...p,description:txt}:p));
    setDescLoad(false);
  };

  const doPrice = async()=>{
    if(!pName||!pCost)return; setPriceLoad(true); setPriceRes(null);
    const txt=await ask([{role:"user",content:`Product: ${pName}. Total landed cost: $${pCost}. Suggest pricing for Somali e-commerce.`}],
      `E-commerce pricing strategist. Return ONLY valid JSON: {"budget":number,"standard":number,"premium":number,"recommendedPrice":number,"margin":"string","demandForecast":"string","tip":"string"}. No markdown.`);
  ...
  const doChat = async()=>{
    ...
    const txt=await ask(nm,`Customer support agent for FrexSOM Market — online store in Somalia. Help with: shipping (15–30 days), payments (EVC Plus, Zaad, E-Dahab, cash), returns (30 days), products. Contact: info@frexsom.com | (+252) 611 013 604. Warm and concise — 2–3 sentences max.`);
    setMsgs(prev=>[...prev,{role:"assistant",content:txt||"Sorry, please call (+252) 611 013 604 for immediate help."}]);
    setChatLoad(false);
  };

  const filtered = products.filter(p=>(catFilter==="all"||p.category===catFilter)&&(searchQ===""||p.name.toLowerCase().includes(searchQ.toLowerCase())||p.category.toLowerCase().includes(searchQ.toLowerCase())));
  ...
  // ════════════════════════════════════════
  // ADMIN PANEL — Only visible after PIN
  // ════════════════════════════════════════
  if(isAdmin) return (
    <div style={{display:"flex",height:"100vh",fontFamily:"'Segoe UI',system-ui,sans-serif",background:GRAY,overflow:"hidden"}}>
      ...
      <div style={{width:220,background:NAVY,display:"flex",flexDirection:"column",flexShrink:0}}>
        <div style={{padding:"20px 16px 14px",borderBottom:"1px solid rgba(255,255,255,0.08)"}}>
          <Logo size={17}/>
          <div style={{fontSize:10,color:ORANGE,marginTop:7,fontWeight:700,letterSpacing:1}}>⚙️ ADMIN PANEL</div>
        </div>
        <nav style={{padding:"12px 10px",flex:1}}>
          {[["⊞","Dashboard","dashboard"],["📋","Orders","orders"],["📦","Catalog","catalog"],["🔎","Source Products","sourcing"],["💲","Pricing","pricing"],["💬","Support Chat","support"]].map(([ic,lbl,pg])=>(
            <button key={pg} onClick={()=>setAdminPage(pg)} style={{display:"flex",alignItems:"center",gap:10,width:"100%",padding:"10px 13px",background:adminPage===pg?`${ORANGE}22`:"transparent",border:adminPage===pg?`1px solid ${ORANGE}55`:"1px solid transparent",borderRadius:9,color:adminPage===pg?ORANGE:"rgba(255,255,255,0.65)",cursor:"pointer",fontSize:13.5,fontWeight:adminPage===pg?700:400,marginBottom:3,textAlign:"left",fontFamily:"inherit"}}>
              <span style={{fontSize:15,width:20,textAlign:"center"}}>{ic}</span>{lbl}
              {pg==="orders"&&orders.filter(o=>o.status==="Processing").length>0&&<span style={{marginLeft:"auto",background:"#ef4444",color:"white",borderRadius:"50%",width:19,height:19,fontSize:11,fontWeight:800,display:"flex",alignItems:"center",justifyContent:"center"}}>{orders.filter(o=>o.status==="Processing").length}</span>}
            </button>
          ))}
        </nav>
        <div style={{padding:"12px 14px",borderTop:"1px solid rgba(255,255,255,0.08)"}}>
          <button onClick={()=>{setIsAdmin(false);showToast("Returned to store","#64748b");}} style={{width:"100%",background:"rgba(239,68,68,0.15)",border:"1px solid rgba(239,68,68,0.3)",color:"#ef4444",padding:"10px",borderRadius:9,cursor:"pointer",fontWeight:700,fontSize:13,fontFamily:"inherit"}}>🚪 Exit to Store</button>
        </div>
      </div>
      ...
      <div style={{flex:1,overflowY:"auto"}}>
        ...
        {adminPage==="dashboard"&&(
          <div style={{padding:30}}>
            <div style={{marginBottom:24}}><div style={{fontSize:24,fontWeight:800,color:NAVY}}>Welcome back, Admin 👋</div><div style={{color:"#7a86a0",fontSize:14,marginTop:3}}>FrexSOM store overview.</div></div>
            <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:14,marginBottom:24}}>
              {[["📦",products.length,"Products",NAVY],["🛒",orders.length,"Orders",GREEN],["💰","~57%","Avg Margin",ORANGE],["❤️",wishlist.length,"Wishlisted","#8b5cf6"]].map(([ic,v,l,c])=>(
                <Card key={l} style={{padding:"18px 20px"}}><div style={{fontSize:26,marginBottom:6}}>{ic}</div><div style={{fontSize:22,fontWeight:800,color:c}}>{v}</div><div style={{fontSize:12,color:"#7a86a0",marginTop:2}}>{l}</div></Card>
              ))}
            </div>
            <div style={{display:"grid",gridTemplateColumns:"2fr 1fr",gap:18}}>
              <Card style={{padding:22}}>
                ...
                {orders.slice(0,4).map((o,i)=>(
                  <div key={i} style={{display:"flex",alignItems:"center",gap:12,padding:"11px 0",borderBottom:i<3?"1px solid #f0f2f8":"none"}}>
                    <div style={{flex:1}}><div style={{fontWeight:700,color:NAVY,fontSize:13}}>{o.name} — {o.city}</div><div style={{fontSize:11.5,color:"#94a3b8"}}>{o.items.substring(0,40)}</div></div>
                    <div style={{textAlign:"right"}}><div style={{fontWeight:800,color:ORANGE,fontSize:13}}>{o.total}</div><span style={{fontSize:11,background:`${statusColor(o.status)}18`,color:statusColor(o.status),padding:"2px 8px",borderRadius:20,fontWeight:700}}>{o.status}</span></div>
                  </div>
                ))}
              </Card>
              <div style={{display:"flex",flexDirection:"column",gap:14}}>
                <Card style={{background:`linear-gradient(135deg,${NAVY},#1a3580)`,padding:22}}>
                  ...
                  {[["🔎 Source Products","sourcing"],["💲 Price a Product","pricing"],["📋 Manage Orders","orders"],["📦 View Catalog","catalog"]].map(([lbl,pg])=>(
                    <button key={pg} onClick={()=>setAdminPage(pg)} style={{display:"block",width:"100%",textAlign:"left",padding:"9px 12px",marginBottom:7,background:"rgba(255,255,255,0.07)",border:"1px solid rgba(255,255,255,0.12)",borderRadius:8,color:"white",cursor:"pointer",fontSize:13,fontWeight:600,fontFamily:"inherit"}}>{lbl}</button>
                  ))}
                </Card>
        ...
        {adminPage==="orders"&&(
          <div style={{padding:30}}>
            <div style={{marginBottom:22}}><div style={{fontSize:22,fontWeight:800,color:NAVY}}>Orders</div><div style={{color:"#7a86a0",fontSize:14}}>{orders.length} total · {orders.filter(o=>o.status==="Processing").length} processing</div></div>
        ...
        {adminPage==="catalog"&&(
          <div style={{padding:30}}>
            <div style={{marginBottom:22}}><div style={{fontSize:22,fontWeight:800,color:NAVY}}>Product Catalog</div><div style={{color:"#7a86a0",fontSize:14}}>{products.length} products</div></div>
                ...
                return (
                  <Card key={p.id} style={{display:"flex",flexDirection:"column",padding:0,overflow:"hidden"}}>
                    ...
                    <div style={{padding:"14px 16px 16px",display:"flex",flexDirection:"column",flex:1}}>
                      <div style={{fontSize:15,fontWeight:700,color:NAVY,marginBottom:4}}>{p.name}</div>
        ...
        {adminPage==="sourcing"&&(
          <div style={{padding:30}}>
            <div style={{marginBottom:22}}><div style={{fontSize:22,fontWeight:800,color:NAVY}}>🔎 Source Products</div><div style={{color:"#7a86a0",fontSize:14}}>Describe what you want to sell — system finds best products and supplier details.</div></div>
            <Card style={{marginBottom:20,padding:24}}>
              <div style={{display:"flex",gap:10,marginBottom:14}}>
                <input value={srcQ} onChange={e=>setSrcQ(e.target.value)} onKeyDown={e=>e.key==="Enter"&&doSource()} placeholder='e.g. "women fashion", "home appliances", "electronics"' style={{flex:1,padding:"12px 18px",borderRadius:9,border:"1.5px solid #dde1ee",fontSize:14,outline:"none",fontFamily:"inherit"}}/>
                <button onClick={doSource} disabled={srcLoad} style={{background:NAVY,color:"white",border:"none",padding:"12px 26px",borderRadius:9,cursor:"pointer",fontWeight:700,fontSize:14,fontFamily:"inherit",opacity:srcLoad?0.7:1,whiteSpace:"nowrap"}}>{srcLoad?"Analyzing…":"🔎 Find"}</button>
              </div>
              <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
                {["Women's Fashion","Electronics","Home Appliances","Beauty & Skincare","Children's Toys","Sports Equipment","Kitchen Essentials"].map(s=>(
            ...
            {srcLoad&&<Card style={{padding:40,textAlign:"center"}}><div style={{fontSize:36,marginBottom:12}}>🔍</div><div style={{fontWeight:700,color:NAVY}}>Finding best products…</div></Card>}
            {srcRes&&srcRes.length===0&&<Card style={{padding:24,textAlign:"center"}}><div style={{color:"#ef4444"}}>No results. Try a different term.</div></Card>}
            {srcRes&&srcRes.length>0&&(
              <div style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:18}}>
                {srcRes.map((p,i)=>(
                  <Card key={i} style={{padding:22}}>
                    <div style={{display:"flex",justifyContent:"space-between",marginBottom:14}}>
                      <div><div style={{fontSize:16,fontWeight:800,color:NAVY}}>{p.name}</div><div style={{fontSize:12,color:"#7a86a0"}}>{p.supplier}</div></div>
        ...
        {adminPage==="pricing"&&(
          <div style={{padding:30}}>
            <div style={{marginBottom:22}}><div style={{fontSize:22,fontWeight:800,color:NAVY}}>Smart Pricing</div><div style={{color:"#7a86a0",fontSize:14}}>Get the optimal selling price for any product.</div></div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:20,alignItems:"start"}}>
              <Card style={{padding:24}}>
                {[["Product Name","text",pName,setPName,"e.g. Solar Power Bank"],["Total Cost in USD","number",pCost,setPCost,"e.g. 12.50"]].map(([lbl,type,val,set,ph])=>(
                  <div key={lbl} style={{marginBottom:16}}><label style={{fontSize:12.5,fontWeight:600,color:"#374151",display:"block",marginBottom:6}}>{lbl}</label><input value={val} onChange={e=>set(e.target.value)} type={type} placeholder={ph} style={{width:"100%",padding:"11px 14px",borderRadius:9,border:"1.5px solid #dde1ee",fontSize:14,boxSizing:"border-box",outline:"none",fontFamily:"inherit"}}/></div>
                ))}
              ...
              {priceRes&&(
                <div>
                  <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10,marginBottom:12}}>
                    {[{l:"Budget",v:`$${priceRes.budget}`,c:"#3b82f6"},{l:"✦ Recommended",v:`$${priceRes.recommendedPrice}`,c:ORANGE},{l:"Premium",v:`$${priceRes.premium}`,c:"#8b5cf6"}].map(x=>(
                  ...
                  <Card style={{padding:18}}>
                    <div style={{display:"flex",gap:10,marginBottom:10}}>
                      ...
                      <div style={{flex:2,background:GRAY,borderRadius:9,padding:"12px 14px"}}><div style={{fontSize:11.5,color:"#7a86a0"}}>Demand Forecast</div><div style={{fontWeight:600,color:NAVY,fontSize:13,marginTop:2}}>{priceRes.demandForecast}</div></div>
                    </div>
                    <div style={{background:LIGHT,border:`1px solid ${ORANGE}33`,borderRadius:9,padding:"11px 14px",fontSize:13,color:"#7a4000"}}>💡 <strong>Tip:</strong> {priceRes.tip}</div>
                  </Card>
                </div>
              )}
            </div>
        ...
        {adminPage==="support"&&(
          <div style={{padding:30,display:"flex",flexDirection:"column",height:"calc(100vh - 60px)"}}>
            <div style={{marginBottom:18}}><div style={{fontSize:22,fontWeight:800,color:NAVY}}>Support Chat Monitor</div><div style={{color:"#7a86a0",fontSize:14}}>See and respond to customer queries.</div></div>
            <Card style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",padding:0}}>
              ...
              <div style={{padding:"13px 20px",borderTop:"1px solid #f0f2f8",display:"flex",gap:10}}>
                <input value={chatIn} onChange={e=>setChatIn(e.target.value)} onKeyDown={e=>e.key==="Enter"&&doChat()} placeholder="Test a customer question…" style={{flex:1,padding:"12px 18px",borderRadius:25,border:"1.5px solid #dde1ee",fontSize:13.5,outline:"none",fontFamily:"inherit"}}/>
                <button onClick={doChat} disabled={chatLoad} style={{background:NAVY,color:"white",border:"none",padding:"12px 22px",borderRadius:25,cursor:"pointer",fontWeight:700,fontSize:13.5,fontFamily:"inherit"}}>Send</button>
              </div>
            </Card>
          </div>
        )}
  ...
  // ════════════════════════════════════════
  // CUSTOMER STOREFRONT — Clean, no admin
  // ════════════════════════════════════════
  return (
    <div style={{fontFamily:"'Segoe UI',system-ui,sans-serif",background:"#f8fafc",minHeight:"100vh"}}>
      ...
      {showAdminLogin&&(
        <div style={{position:"fixed",inset:0,zIndex:200,background:"rgba(13,31,92,0.75)",display:"flex",alignItems:"center",justifyContent:"center"}}>
          <div style={{background:"white",borderRadius:18,width:360,padding:36,textAlign:"center",boxShadow:"0 24px 64px rgba(0,0,0,0.3)"}}>
            ...
            <div style={{fontSize:18,fontWeight:800,color:NAVY,marginBottom:4}}>Admin Access</div>
            <div style={{fontSize:13,color:"#7a86a0",marginBottom:22}}>Enter your PIN</div>
            <input type="password" value={adminPin} onChange={e=>setAdminPin(e.target.value)} onKeyDown={e=>e.key==="Enter"&&tryAdminLogin()} placeholder="• • • •" maxLength={6}
            ...
            <div style={{display:"flex",gap:10}}>
              <button onClick={()=>{setShowAdminLogin(false);setAdminPin("");}} style={{flex:1,background:"#f0f2f8",color:"#64748b",border:"none",padding:"12px",borderRadius:10,cursor:"pointer",fontWeight:700,fontFamily:"inherit"}}>Cancel</button>
              <button onClick={tryAdminLogin} style={{flex:1,background:NAVY,color:"white",border:"none",padding:"12px",borderRadius:10,cursor:"pointer",fontWeight:700,fontFamily:"inherit"}}>Enter →</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div style={{position:"fixed",inset:0,zIndex:80,background:"rgba(13,31,92,0.65)",display:"flex",alignItems:"center",justifyContent:"center"}} onClick={()=>setQuickView(null)}>
          <div style={{background:"white",borderRadius:18,width:600,overflow:"hidden",boxShadow:"0 24px 64px rgba(0,0,0,0.25)"}} onClick={e=>e.stopPropagation()}>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr"}}>
              <img src={quickView.image} alt={quickView.name} style={{width:"100%",height:340,objectFit:"cover"}} onError={e=>{e.target.src="https://images.unsplash.com/photo-1523275335684-37898b6baf30?w=600&q=80";}}/>
              <div style={{padding:26,display:"flex",flexDirection:"column"}}>
                ...
                <div style={{fontSize:19,fontWeight:800,color:NAVY,marginBottom:8,flex:1}}>{quickView.name}</div>
      ...
      {/* Customer Navbar */}
      <div style={{background:NAVY,padding:"0 28px",position:"sticky",top:0,zIndex:20,boxShadow:"0 2px 16px rgba(0,0,0,0.2)"}}>
        <div style={{display:"flex",alignItems:"center",gap:16,height:68}}>
          ...
          <div style={{display:"flex",gap:8,alignItems:"center",marginLeft:"auto"}}>
            <a href="tel:+252611013604" style={{color:"rgba(255,255,255,0.75)",textDecoration:"none",fontSize:12.5,fontWeight:600,display:"flex",alignItems:"center",gap:4}}>📞 Help</a>
            <button onClick={()=>setMyOrders(!myOrders)} style={{background:"rgba(255,255,255,0.1)",border:"1px solid rgba(255,255,255,0.2)",color:"white",padding:"8px 14px",borderRadius:25,cursor:"pointer",fontWeight:600,fontSize:12.5,fontFamily:"inherit"}}>📋 My Orders</button>
      ...
      {myOrders&&(
        <div style={{background:"white",borderBottom:"2px solid #eaecf4",padding:"20px 28px"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
            <div style={{fontSize:16,fontWeight:800,color:NAVY}}>📋 My Recent Orders</div>
            <button onClick={()=>setMyOrders(false)} style={{background:"none",border:"none",cursor:"pointer",fontSize:18,color:"#94a3b8"}}>✕</button>
          </div>
          {orders.length===0
            ?<div style={{color:"#94a3b8",textAlign:"center",padding:"20px 0"}}>No orders yet. Start shopping!</div>
            :orders.slice(0,3).map((o,i)=>(
              <div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 0",borderBottom:i<2?"1px solid #f0f2f8":"none"}}>
                <div><div style={{fontWeight:700,color:NAVY,fontSize:13}}>{o.id}</div><div style={{fontSize:12,color:"#94a3b8"}}>{o.items.substring(0,40)}</div></div>
                <div style={{textAlign:"right"}}><div style={{fontWeight:700,color:ORANGE}}>{o.total}</div><span style={{fontSize:11,background:`${statusColor(o.status)}18`,color:statusColor(o.status),padding:"2px 8px",borderRadius:20,fontWeight:700}}>{o.status}</span></div>
              </div>
            ))
          }
        </div>
      ...
      {!searchQ&&(
        <div style={{background:`linear-gradient(130deg,${NAVY} 0%,#1a3580 55%,${ORANGE}aa 100%)`,padding:"52px 40px",color:"white",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <div style={{maxWidth:560}}>
            ...
            <div style={{fontSize:14,opacity:0.85,marginBottom:20,lineHeight:1.6}}>Quality products delivered to your door across Somalia. Pay with EVC Plus, Zaad or cash.</div>
        ...
        <div style={{background:"white",borderBottom:"1px solid #eaecf4",padding:"14px 32px"}}>
          <div style={{display:"flex",justifyContent:"space-around",flexWrap:"wrap",gap:12}}>
            {[["🚢","Fast Delivery","15–30 days nationwide"],["💳","Mobile Payments","EVC Plus, Zaad & E-Dahab"],["🔄","Easy Returns","30-day return policy"],["🎧","24/7 Support","Always here to help"],["🔒","Secure Orders","Safe & trusted checkout"]].map(([ic,t,s])=>(
              <div key={t} style={{display:"flex",alignItems:"center",gap:10}}><span style={{fontSize:24}}>{ic}</span><div><div style={{fontWeight:700,color:NAVY,fontSize:13}}>{t}</div><div style={{fontSize:11.5,color:"#94a3b8"}}>{s}</div></div></div>
            ))}
          </div>
        </div>
      )}
      ...
      {!searchQ&&(
        <div style={{margin:"0 28px 32px",borderRadius:16,overflow:"hidden",background:`linear-gradient(135deg,${ORANGE},#e67e00)`,padding:"28px 36px",display:"flex",justifyContent:"space-between",alignItems:"center",color:"white"}}>
          <div>
            ...
            <div style={{fontSize:14,opacity:0.9,marginBottom:16}}>Use code <strong>FREXSOM</strong> at checkout</div>
      ...
      <div ref={productsRef} style={{padding:"0 28px 32px"}}>
        ...
        {filtered.length===0
          ?<div style={{textAlign:"center",padding:60,color:"#94a3b8"}}><div style={{fontSize:52}}>🔍</div><div style={{fontWeight:700,fontSize:16,color:NAVY,marginBottom:8}}>No products found</div><button onClick={clearSearch} style={{background:NAVY,color:"white",border:"none",padding:"10px 24px",borderRadius:25,cursor:"pointer",fontWeight:700,fontFamily:"inherit"}}>Browse All</button></div>
          :<div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:20}}>{filtered.map(p=><ProductCard key={p.id} p={p}/>)}</div>
        }
      </div>
      ...
      <div style={{margin:"0 28px 32px",background:NAVY,borderRadius:16,padding:"36px 40px",display:"flex",justifyContent:"space-between",alignItems:"center",gap:20,flexWrap:"wrap"}}>
        <div><div style={{fontSize:22,fontWeight:800,color:"white",marginBottom:6}}>📧 Stay in the Loop</div><div style={{fontSize:14,color:"rgba(255,255,255,0.6)"}}>Get exclusive deals and new arrivals.</div></div>
        {newsletterDone
      ...
      <div style={{background:NAVY,color:"rgba(255,255,255,0.55)",padding:"44px 36px 24px"}}>
        <div style={{display:"grid",gridTemplateColumns:"1.6fr 1fr 1fr 1fr",gap:30,marginBottom:32}}>
          ...
          {[
            {title:"Quick Links",items:[["Shop All",()=>setCatFilter("all")],["My Orders",()=>setMyOrders(true)],["Contact Support",()=>window.location.href="mailto:info@frexsom.com"]]},
        ...
        <div style={{position:"fixed",inset:0,zIndex:50,display:"flex"}}>
          ...
          <div style={{width:400,background:"white",display:"flex",flexDirection:"column",boxShadow:"-4px 0 32px rgba(0,0,0,0.15)"}}>
            <div style={{padding:"18px 22px",borderBottom:"1px solid #f0f2f8",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              ...
              <button onClick={()=>setCartOpen(false)} style={{background:"#f0f2f8",border:"none",borderRadius:"50%",width:32,height:32,cursor:"pointer",fontSize:17,display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"inherit"}}>✕</button>
            </div>
            {cart.length===0
              ?<div style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",color:"#94a3b8"}}><div style={{fontSize:56}}>🛒</div><div style={{fontWeight:700,marginTop:12,color:NAVY}}>Your cart is empty</div><button onClick={()=>setCartOpen(false)} style={{marginTop:18,background:NAVY,color:"white",border:"none",padding:"10px 24px",borderRadius:9,cursor:"pointer",fontWeight:700,fontFamily:"inherit"}}>Browse Products</button></div>
              :<>
                <div style={{flex:1,overflowY:"auto",padding:"14px 22px"}}>
                  {cart.map(item=>(
                    <div key={item.id} style={{display:"flex",alignItems:"center",gap:12,padding:"13px 0",borderBottom:"1px solid #f4f6fb"}}>
                      <img src={item.image} alt={item.name} style={{width:54,height:54,borderRadius:10,objectFit:"cover",flexShrink:0}} onError={e=>{e.target.src="https://images.unsplash.com/photo-1523275335684-37898b6baf30?w=600&q=80";}}/>
                      <div style={{flex:1}}><div style={{fontSize:13.5,fontWeight:700,color:NAVY}}>{item.name}</div><div style={{fontSize:13,color:ORANGE,fontWeight:700,marginTop:3}}>${(item.price*item.qty).toFixed(2)}</div></div>
          ...
          <div style={{background:"white",borderRadius:18,width:490,maxHeight:"92vh",overflowY:"auto",boxShadow:"0 24px 64px rgba(0,0,0,0.22)"}}>
            <div style={{padding:"26px 28px 0"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:22}}>
                ...
                <button onClick={()=>setCheckoutStep(null)} style={{background:"#f0f2f8",border:"none",borderRadius:"50%",width:34,height:34,cursor:"pointer",fontSize:17,display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"inherit"}}>✕</button>
              </div>
              {[["Full Name","text","name","e.g. Amina Hassan"],["Phone Number","tel","phone","e.g. (+252) 611 000 000"],["Street Address","text","address","Street, district, neighbourhood"]].map(([lbl,type,key,ph])=>(
                <div key={key} style={{marginBottom:15}}>
                  ...
                  <input value={form[key]} onChange={e=>setForm(p=>({...p,[key]:e.target.value}))} type={type} placeholder={ph} style={{width:"100%",padding:"11px 14px",borderRadius:9,border:`1.5px solid ${form[key]?GREEN:"#dde1ee"}`,fontSize:13.5,boxSizing:"border-box",outline:"none",fontFamily:"inherit"}}/>
                </div>
              ))}
              ...
              <div style={{fontWeight:700,color:NAVY,fontSize:13.5,marginBottom:12,marginTop:20}}>Payment Method</div>
            ...
            <div style={{padding:"0 28px 26px",display:"flex",gap:10}}>
              ...
              <button onClick={placeOrder} disabled={!form.name||!form.phone} style={{flex:2,background:(!form.name||!form.phone)?"#c0c9dd":ORANGE,color:"white",border:"none",padding:"13px",borderRadius:10,cursor:"pointer",fontWeight:800,fontSize:15,fontFamily:"inherit"}}>✓ Place Order</button>
            </div>
          </div>
        </div>
      )}
      ...
      {checkoutStep==="done"&&orderDone&&(
        <div style={{position:"fixed",inset:0,zIndex:60,background:"rgba(13,31,92,0.55)",display:"flex",alignItems:"center",justifyContent:"center"}}>
          <div style={{background:"white",borderRadius:18,width:450,padding:36,textAlign:"center",boxShadow:"0 24px 64px rgba(0,0,0,0.22)"}}>
            ...
            <div style={{fontSize:14,color:"#7a86a0",marginBottom:22}}>Shukran! Your order has been placed.</div>
            ...
            <div style={{fontSize:13,color:"#7a86a0",marginBottom:20}}><a href="mailto:info@frexsom.com" style={{color:ORANGE,fontWeight:600,textDecoration:"none"}}>info@frexsom.com</a> · <a href="tel:+252611013604" style={{color:ORANGE,fontWeight:600,textDecoration:"none"}}>(+252) 611 013 604</a></div>
